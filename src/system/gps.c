...
static double hdop = 9999;
static double dAltitude = 9000;
...
void GpsConvertHDOPFromStringToNumerical(void) {
	hdop = 0;
	for (int i = 0; i < 4; i++) {
		if (NmeaGpsData.NmeaHorizontalDilution[i] == '.') {
			hdop += 0.1 * (NmeaGpsData.NmeaHorizontalDilution[i + 1] - '0');
			break;
		} else if (NmeaGpsData.NmeaHorizontalDilution[i] >= '0'
				&& NmeaGpsData.NmeaHorizontalDilution[i] <= '9') {
			hdop = 10 * hdop + (NmeaGpsData.NmeaHorizontalDilution[i] - '0');
		} else {
			hdop = 9999;
			break;
		}
...
void GpsConvertAltitudeFromStringToNumerical(void) {
	dAltitude = 0;
	for (int i = 0; i < 6; i++) {
		if (NmeaGpsData.NmeaAltitude[i] == '.') {
			dAltitude += 0.1 * (NmeaGpsData.NmeaAltitude[i + 1] - '0');
			break;
		} else if (NmeaGpsData.NmeaAltitude[i] >= '0'
				&& NmeaGpsData.NmeaAltitude[i] <= '9') {
			dAltitude = 10 * dAltitude + (NmeaGpsData.NmeaAltitude[i] - '0');
		} else {
			dAltitude = 9500;
			break;
		}
...
uint8_t GpsParseGpsData(int8_t *rxBuffer, int32_t rxBufferSize) {
	...
	if (strncmp((const char*) NmeaGpsData.NmeaDataType,
			(const char*) NmeaDataTypeGPGGA, 5) == 0) {
		// NmeaUtcTime
		fieldSize = 0;
		while (rxBuffer[i + fieldSize++] != ',') {
			if (fieldSize > 11) {
...
void GpsFormatGpsData(void) {
	...
	GpsConvertHDOPFromStringToNumerical();
